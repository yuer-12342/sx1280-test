 ...
 */
#define DEMO_FREQ_HOPPED_RANGING            0
...
#if ( DEMO_FREQ_HOPPED_RANGING == 0 )
/* 单信道测距：结果缓冲不必按 255 信道分配（HC32L021 RAM 仅 6KB） */
#define DEMO_RNG_RESULTS_BUF_MAX              64
#else
#define DEMO_RNG_RESULTS_BUF_MAX              DEMO_RNG_CHANNELS_COUNT_MAX
#endif
...
/*!
 * \brief Continuous ranging (fence core step-1): samples per round and hop budget.
 */
#define RNG_CONTINUOUS_SAMPLE_COUNT       5u
#define RNG_CONTINUOUS_HOP_FACTOR         10u
#define RNG_CONTINUOUS_SESSION_GAP_MS     1000u

typedef enum
{
    RNG_OP_MODE_DEMO = 0,
    RNG_OP_MODE_CONTINUOUS,
} RangingDemoOpMode_t;
...
typedef struct{
    ...
    int RngResultIndex;
    double RawRngResults[DEMO_RNG_RESULTS_BUF_MAX];
    double RngResults[DEMO_RNG_RESULTS_BUF_MAX];
    int8_t RawRngRssi[DEMO_RNG_RESULTS_BUF_MAX];
}DemoResult_t;
...
/*!
 * \brief Enable continuous short-session ranging (no button, auto re-arm).
 */
void RangingDemoSetContinuousMode( uint8_t enable );

/*!
 * \brief Prepare the next quick ranging round (shorter gap than RangingDemoReset).
 */
